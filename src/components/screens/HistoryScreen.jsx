...
const STATUS_LABELS = {
  new:       'Membre',
  returning: 'Client fidele',
  preferred: 'Preferred Guest',
  vip:       'VIP',
}

export default function HistoryScreen({ reservation, session }) {
  ...
  return (
    <div className="page">
      <Header/>
      <div className="ptitle">Apres sejour</div>

      <div style={{ padding: '0 var(--px) 12px' }}>
        <div className="lcard">
          <div className="lbdg">
            <Icon name="sparkles" size={12} color="var(--gold)"/>
            {STATUS_LABELS[profile?.status || 'new']}
          </div>
          <div className="ltit">
            Bonjour, {session?.user?.email?.split('@')[0] || 'cher client'}
          </div>
          <div className="lsub">
            {profile?.total_stays || 0} sejour{(profile?.total_stays || 0) > 1 ? 's' : ''} · Membre Your Home
          </div>
      ...
      {reservation && (
        <>
          <div className="slbl">Sejour actuel</div>
            ...
            <div style={{ padding: '14px 18px' }}>
              {review ? (
                ...
                <button className="btn-p" onClick={() => setShowReview(true)}>
                  <Icon name="star" size={13} color="#fff"/> Laisser un avis
                </button>
              )}
            </div>
          </div>
      ...
      {past.length > 0 && (
        <>
          <div className="slbl">Sejours passes</div>
          {past.map((s, i) => (
            <div key={i} className="stcard">
              ...
              <div className="stbd">
                <div className="stn">Logement Your Home</div>
                <div className="stm">
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                  ...
                  <button className="btn-p" style={{ fontSize: 9, padding: '10px 8px' }}>
                    <Icon name="repeat" size={11} color="#fff"/> Reserver
                  </button>
                </div>
              </div>
            </div>
      ...
      {past.length === 0 && !reservation && (
        <div style={{ padding: '40px var(--px)', textAlign: 'center' }}>
          <div style={{ fontSize: 13, color: 'var(--muted)', fontWeight: 300, lineHeight: 1.6 }}>
            Votre historique de sejours apparaitra ici.
          </div>
        </div>
      )}
